...
select.smpl <- c(8,2,12,16,14,13,6,18)
...
dir.create(paste0(result.wd,"/Figures/S2_SeqLogo"))
for(cm.num in cm.all) {
  ...
  p2.logo.list <- lapply(select.smpl,
                         function(w.smpl) {
                           ...
                           wm <- read.table(paste0(data.wd, "Data/", virus.file.name, "/",
                                                   mix.name, "/Wm.txt"),
  ...
  if((p.high * 2) <= a4h) {
    ...
    ggsave(paste0("FigureS2_FullLogo_", cm, ".png"), p2.logo.pannel,
           path = paste0(result.wd,"/Figures/S2_SeqLogo"),
           width = a4w, height = p.high * 2, units = "mm", dpi = "retina",
           device = "png", limitsize = FALSE)
    
  } else {
    #Save figure with combined plots
    ggsave(paste0("FigureS2_FullLogo_", cm, "_A.png"), p2.logo.1,
           path = paste0(result.wd,"/Figures/S2_SeqLogo"),
           width = a4w, height = p.high, units = "mm", dpi = "retina",
           device = "png", limitsize = FALSE)
    
    ggsave(paste0("FigureS2_FullLogo_", cm, "_B.png"), p2.logo.2,
           path = paste0(result.wd,"/Figures/S2_SeqLogo"),
           width = a4w, height = p.high, units = "mm", dpi = "retina",
           device = "png", limitsize = FALSE)
