Code files can be sorted into three categories:
1) pre-mixture model code - generates integration site ranges and the final outputs are a tables of the numeric-coded sequences that serve as an input for mixture model
2) mixture model - runs EM algorithm and outputs reports from which data describing the mixture components can be extracted
3) post-mixture model code - code that extract the information about the mixture components
4) HotSpot code - code creating the data for the analysis of the HIV hotspot targeting
5) Figures - R code that creates the Figures found in the publication
...
add 1)
	Get_IS_coordinates.sh - describes how coordinates of IS are obtained
...
add 4)
	CompSeq - directory with R code that identifies a selects the sequences associted with given mixture component
...
add 5)
	Directory contains R code used to create Figures present in the publication. The code uses data created by the code from categories 2) - 4).
